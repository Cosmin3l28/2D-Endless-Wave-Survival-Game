...
class Arena:
    ...
    def render_background(self):
        ...
        self.game.screen.blit(
            self.game.assets["background"],
            (
                0 - self.game.camera.camera_scroll[0],
                0 - self.game.camera.camera_scroll[1],
            ),
        )  # draw the background
        ...
        self.arena_rect.topleft = (
            arena_position[0] - self.game.camera.camera_scroll[0],
            arena_position[1] - self.game.camera.camera_scroll[1],
        )
        pygame.draw.rect(
            self.game.screen, (255, 0, 0), self.arena_rect, 1
        )  # draw the arena rectangle for debugging

    def handle_collision(self):
        # Check for collision with the arena boundaries
        if (
            self.game.player.position[0]
            < self.arena_rect.left + self.game.camera.camera_scroll[0]
        ):
            self.game.player.position[0] = (
                self.arena_rect.left + self.game.camera.camera_scroll[0]
            )
        if (
            self.game.player.position[0] + self.game.player.size[0]
            > self.arena_rect.right + self.game.camera.camera_scroll[0]
        ):
            self.game.player.position[0] = (
                self.arena_rect.right
                + self.game.camera.camera_scroll[0]
                - self.game.player.size[0]
            )
        if (
            self.game.player.position[1]
            < self.arena_rect.top + self.game.camera.camera_scroll[1]
        ):
            self.game.player.position[1] = (
                self.arena_rect.top + self.game.camera.camera_scroll[1]
            )
        if (
            self.game.player.position[1] + self.game.player.size[1]
            > self.arena_rect.bottom + self.game.camera.camera_scroll[1]
        ):
            self.game.player.position[1] = (
                self.arena_rect.bottom
                + self.game.camera.camera_scroll[1]
                - self.game.player.size[1]
            )
...
class Camera:
    ...
    def update_camera_scroll(self):
        self.camera_scroll[0] += (
            self.game.player.rectangle().centerx
            - int(self.width / 2)
            - self.camera_scroll[0]
        )
        self.camera_scroll[1] += (
            self.game.player.rectangle().centery
            - int(self.height / 2)
            - self.camera_scroll[1]
        )
        ...
        if (
            self.camera_scroll[0]
            > self.game.assets["background"].get_width() - self.width
        ):
            self.camera_scroll[0] = (
                self.game.assets["background"].get_width() - self.width
            )
        if (
            self.camera_scroll[1]
            > self.game.assets["background"].get_height() - self.height
        ):
            self.camera_scroll[1] = (
                self.game.assets["background"].get_height() - self.height
            )
...
class Animation:
    ...
    def animate(self, min_frame_index=0, max_frame_index=None):
        ...
        if self.loop:
            self.game_frame_index = (self.game_frame_index + 1) % (
                self.frame_duration * max_frame_index
            ) + min_frame_index
        else:
            self.game_frame_index = min(
                self.game_frame_index + 1, self.frame_duration * max_frame_index - 1
            )
...
class Stamina_Bar:
    ...
    def render(self):
        ...
        pygame.draw.rect(
            self.game.screen,
            self.color,
            (self.position[0], self.position[1], width, self.size[1]),
        )

    def stamina_regen(self):
        if self.game.player.running == False:
            if self.current_stamina < self.max_stamina:
                self.current_stamina += 5 / self.game.framerate  
                current_action = self.game.player.get_action()
                if current_action == " idle" or "idle_up" or "idle_down":
                    self.current_stamina += 5 / self.game.framerate
                if self.current_stamina > self.max_stamina:
